...
\todo{figure of executors in worker with receive and send queue}
\todo{add arrows to other executors}
...
Additionally, every worker has a heartbeat thread that simulates sending heartbeat messages to the Nimbus process. It does this by writing to a local cache which is persisted to a file by a blocking write on every heartbeat.
...
\begin{minipage}{.5\textwidth}
  ...
  \captionof{figure}{Worker node in Apache Storm.}
  \label{fig:test1}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
  ...
  \captionsetup{width=0.9\linewidth}
  \captionof{figure}{Worker "node" in Storm-MC.}
  \label{fig:test2}
\end{minipage}
\end{figure}
