...
The first topology we tested for performance is a variant of the aforementioned WordCount topology. Recall, that this topology is shown graphically in Figure \ref{fig:wordcount_topology}. Since the components do not store any data in memory or make any I/O calls this topology is mostly CPU-bound.
...
Of note, the resident size used by Storm-MC is also about half of resident size used by Apache Storm. This could be explained by all the additional memory Storm has to allocate for inter-node communication.

\subsection{Enron Topology}

Next, we tested the Enron topology from the IBM benchmarks. In this topology, serialised emails from the Enron email dataset are read from a file by a \texttt{ReadEmailsDecompressSpout} spout. They are then deserialised by \texttt{AvroDeserializeBolt}, filtered by \texttt{NewFilterBolt}, modified by \texttt{ModifyBolt}, and finally metrics are recorded by the \texttt{NewMetricsBolt}. Additionally, executors running this bolt send their average email throughput every four seconds to one global \texttt{GlobalMetricsBolt} (excluded from the parallelism setting) which records the global average email throughput.

Similarly to the WordCount topology, this topology is serial in nature. However, whereas the WordCount topology keeps a small number of sentences in memory, the Enron topology has a spout that produces tuples by reading from a file. Thus, this benchmark is mostly I/O-bound. The average email throughput in Storm-MC and Apache Storm is shown in tables \ref{table:storm_mc_enron} and \ref{table:storm_enron}, respectively. 
...
As can be seen from the tables, the difference in throughput in Enron Topology is less staggering than in WordCount. We believe this is due to the fact that the throughput is limited by the file reads in the spout. However, as the parallelism increases the improvement in throughput of Storm-MC becomes more apparent as shown in Figure \ref{fig:enron-plot}. This figure also shows that the throughput is fairly volatile. This could be due to the fact that the file is loaded into main memory in chunks and hence the throughput drops when the spout is trying to read from a file in between these chunks.

\subsection{RollingSort Topology}

The RollingSort topology was ported over from the aforementioned storm-benchmark project. This topology only includes one spout and one bolt. The spout produces hundred character-long strings of random digits from zero to eight. The bolt stores a rolling window of hundred of these messages and sorts them every 10 seconds. This benchmark is considered to be memory-bound: a bolt stores a window of tuples in memory and performs a non-linear time sort. The results of running this benchmark on Storm-MC and Apache Storm can be seen in tables \ref{table:storm_mc_rolling} and \ref{table:storm_rolling}.
...
\begin{tabular}{@{}rccccl@{}}
    ...
    7 & {395,386,336} & {395,309,900} & {\%} & {} \\
    8 & {366,680,402} & {366,553,300} & {\%} & {} \\
    9 & {359,091,633} & {358,942,200} & {\%} & {} \\
    10 & {313,912,451} & {313,811,300} & {\%} & {} \\
\end{tabular}
